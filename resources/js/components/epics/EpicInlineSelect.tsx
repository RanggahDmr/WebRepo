...
export default function EpicInlineSelect({ epicUuid, field, value, options, displayLabel }: Props) {
...
const shownLabel = shownMeta?.label ?? displayLabel ?? (shownValue == null ? "-" : String(shownValue));
const shownColor = shownMeta?.color ?? null;
  ...
  function save(nextVal: number | null) {
    ...
    router.patch(
      ...
      {
        preserveScroll: true,
        only: ["epics", "epicStatuses", "epicPriorities"],
        onError: () => {
  ...
  if (!editing) {
    return (
      <button type="button" className="block w-full text-left" onClick={() => setEditing(true)}>
        <Badge variant={shownLabel} color={shownColor}>{shownLabel}</Badge>
      </button>
    );
  }
