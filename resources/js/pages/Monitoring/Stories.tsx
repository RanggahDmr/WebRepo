...
export default function MonitoringStoriesIndex() {
  const { stories, filters, roles, boards, epics, progress }: any = usePage().props;

  const items: Story[] = stories?.data ?? stories ?? [];
  ...
  useEffect(() => {
    ...
    const exists = items.some((s) => s.uuid === selectedStory.uuid);
    if (!exists) {
  ...
  return (
    <AuthenticatedLayout
      header={<h1 className="text-xl font-semibold">Monitoring Stories</h1>}
          ...
          <tbody className="divide-y">
            {items.map((s) => {
              ...
              return (
                ...
                >
                  <td className="px-6 py-3 font-medium text-blue-600">{s.code}</td>
                  ...
                  <td className="px-6 py-3 text-gray-700">{formatDateTime(s.created_at)}</td>
                  <td className="px-6 py-3 text-gray-700">{formatDateTime(s.updated_at)}</td>
